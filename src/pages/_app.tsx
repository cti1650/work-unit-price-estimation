import 'tailwindcss/tailwind.css';

import React from 'react';
import App, { AppProps } from 'next/app';
import Layout from '@comp/layout/layout';
import Head from 'next/head';
import { usePageView } from '@hooks/usePageView';

const TailwindApp = (props: AppProps) => {
  const { Component, pageProps } = props;
  usePageView();
  return (
    <Layout>
      <Head>
        <title>Work Unit Price Estimation</title>
        <link rel='icon' href='/icon/tailwind_cc-128x128.ico' />
        <meta name='viewport' content='user-scalable=no' />
        <meta
          name='viewport'
          content='width=device-width,initial-scale=1.0,minimum-scale=1.0'
        ></meta>
        <meta
          name='description'
          content='作業の見積や改善効果の確認など作業に対してどのくらい時間とコストがかかるか計算するためのツールです！'
        />
        <meta name='keywords' content='HTML,CSS,Next.js,Tailwind.css'></meta>
        <meta property='og:title' content='Work Unit Price Estimation' />
        <meta
          property='og:description'
          content='作業の見積や改善効果の確認など作業に対してどのくらい時間とコストがかかるか計算するためのツールです！'
        />
        <meta property='og:type' content='website' />
        <meta property='og:url' content='https://work-unit-price-estimation.vercel.app/' />
        <meta property='og:image' content='/icon/tailwind_cc-128x128.ico' />
        <meta property='og:site_name' content='Work Unit Price Estimation' />
        <meta property='og:locale' content='ja_JP' />
      </Head>
      <Component {...pageProps} />
    </Layout>
  );
};

export default TailwindApp;
